Shader "Else/Snow/SimpleSnow"
{
    Properties
    {
        ...
        _SnowDiffTex ("SnowDiffTex", 2d) = "white" {}
        _SnowNormalTex ("SnowNormalTex", 2d) = "bump" {}
        _SnowRoughnessTex ("SnowRoughnessTex", 2d) = "white" {}
        _Tint("Tint", Color) = (1 ,1 ,1 ,1)
		[Gamma] _Metallic("Metallic", Range(0, 1)) = 0
		_Smooth ("Smooth", Range(0, 0.94)) = 0.5
        _LocalNormal ("LocalNormal", range(0, 1)) = 0.5
        _SnowStrength ("SnowStength", range(0, 1)) = 0.5
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_SnowDiffTex); SAMPLER(sampler_SnowDiffTex);
            TEXTURE2D(_SnowNormalTex); SAMPLER(sampler_SnowNormalTex);
            TEXTURE2D(_SnowRoughnessTex); SAMPLER(sampler_SnowRoughnessTex);
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                //通过法线的Y值判断雪的差值
                float snowLerp = saturate(dot(finiNormal.y, _SnowStrength));
                float3 snowWorldNormalTex = mul(UnpackNormal(SAMPLE_TEXTURE2D(_SnowNormalTex, sampler_SnowNormalTex, i.uv)), tangentTransform);
                finiNormal = lerp(finiNormal, snowWorldNormalTex, snowLerp);
                ...
                //融合雪Diff
                float3 snowDiff = SAMPLE_TEXTURE2D(_SnowDiffTex, sampler_SnowDiffTex, i.uv).rgb;
                Albedo = lerp(Albedo, snowDiff, snowLerp);
