...
public class GrainyBlurPostProcessing : ScriptableRendererFeature
{
    [System.Serializable]
    ...
    private class BlitPass : ScriptableRenderPass
    {
        ...
        private GrainyBlurPostProcessingVolume grainyBlurPostProcessingVolume;
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            grainyBlurPostProcessingVolume = stack.GetComponent<GrainyBlurPostProcessingVolume>();

            if (grainyBlurPostProcessingVolume == null)
            {
                Debug.LogError("can't get volume");
                return;
            }
            
            if (!grainyBlurPostProcessingVolume.IsActive())
            {
                return;
            }
        ...
        private void Render(CommandBuffer cmd, RenderingData renderingData)
        {
            ...
            int rtW = w / grainyBlurPostProcessingVolume.DownSample.value;
            int rtH = h / grainyBlurPostProcessingVolume.DownSample.value;
            
            cmd.GetTemporaryRT(buffer0, rtW,rtH,0, FilterMode.Bilinear);
            cmd.Blit(source,buffer0);
            
            cmd.SetGlobalVector(Params, new Vector2(grainyBlurPostProcessingVolume.BlurOffset.value / h, grainyBlurPostProcessingVolume.BlurTimes.value));
