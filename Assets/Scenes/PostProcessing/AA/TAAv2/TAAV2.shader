Shader "PostProcessing/AA/TAAV2"
{
    Properties
    {
    ...
    float3 _TAAParams; // xy = offset, z = feedback
    TEXTURE2D_X(_MainTex);
    uniform float4 _MainTex_TexelSize;

    TEXTURE2D_X(_TAAPrevRT);
    TEXTURE2D_X_FLOAT(_CameraDepthTexture);
    float4x4 _TAAPrevVP;
    float4x4 _TAACurInvProject;
    float4x4 _TAACurInvView;
    ...
    VaryingTAA VertexTAA(AttributesTAA input)
    {
        VaryingTAA output;
    ...
    float2 historyPostion(float2 un_jitted_uv)
    {
        ...
        #endif
        float3 viewPos = ComputeViewSpacePosition(un_jitted_uv, depth, _TAACurInvProject);
        float4 worldPos = float4(mul(unity_CameraToWorld, float4(viewPos, 1.0)).xyz, 1.0);

        float4 historyNDC = mul(_TAAPrevVP, worldPos);
    ...
    float4 Frag(VaryingTAA input) : SV_Target
    {
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
        float2 uv_jitted = UnityStereoTransformScreenSpaceTex(input.uv);
        float2 un_jitted_uv = uv_jitted - _TAAParams.xy;
        ...
        float4 prev_color = SAMPLE_TEXTURE2D_X(_TAAPrevRT, sampler_LinearClamp, previousTC);
        prev_color = clip_aabb(color_min, color_max, color_avg, prev_color);
        float4 result_color = lerp(color, prev_color, _TAAParams.z);
        return result_color;
    }
    ENDHLSL
